...
As we know for every query object defined in the queryArray we get a hook respectively. In this we will learn about the query hook.
...
```jsx
const { loading, error, reload, data } = useGetAllPost();
```
